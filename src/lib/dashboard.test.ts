...
describe("chaveMes", () => {
  test("formata ano-mês com zero à esquerda, em UTC", () => {
    expect(chaveMes(new Date(Date.UTC(2026, 0, 15)), "UTC")).toBe("2026-01");
    expect(chaveMes(new Date(Date.UTC(2026, 10, 1)), "UTC")).toBe("2026-11");
  });
});

describe("rotuloMes", () => {
  test("abrevia mês em pt-BR com ano de 2 dígitos", () => {
    expect(rotuloMes(new Date(2026, 0, 1), "UTC")).toMatch(/^Jan\/26$/i);
  });
});
...
describe("mesesJanela", () => {
  test("gera N meses terminando no mês de referência, em ordem crescente", () => {
    ...
    const meses = mesesJanela(referencia, 6, "UTC");
  ...
  test("quantidade 1 retorna só o mês de referência", () => {
    const meses = mesesJanela(new Date(2026, 3, 1), 1, "UTC");
...
describe("bucketizarPorMes", () => {
  const meses = mesesJanela(new Date(2026, 2, 1), 3, "UTC"); // Jan, Fev, Mar/2026
  ...
  test("conta itens no mês correto", () => {
    ...
    const contagem = bucketizarPorMes(itens, meses, (i) => i.data, "UTC");
  ...
  test("meses sem nenhum item ficam em zero (não ausentes)", () => {
    const contagem = bucketizarPorMes([], meses, (i: { data: Date }) => i.data, "UTC");
